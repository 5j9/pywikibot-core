...
def install_packages(python_dir, python_ver):
    ...
    if python_ver < '2.7.9':
        ...
        packages.remove('setuptools')  # can't upgraded bootstrapped setuptools
        packages.extend((
            'urllib3[secure]', 'wheel', 'pip'))
        # packages = list(filter(lambda p: 'setuptools' not in p, download_packages(packages, python_dir)))
