...
class HarvestRobot(WikidataBot):
    ...
    def getTemplateSynonyms(self, title):
        ...
        titles = [page.title(with_namespace=False)
                  for page in temp.getReferences(redirects_only=True,
                                                 namespaces=[10],
                                                 follow_redirects=False)]
        titles.append(temp.title(with_namespace=False))
        return titles
        ...
        for (template, fielddict) in templates:
            ...
            try:
                template = pywikibot.Page(page.site, template,
                                          ns=10).title(with_namespace=False)
            except pywikibot.exceptions.InvalidTitle:
                pywikibot.error(
