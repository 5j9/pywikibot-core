...
class CommonscatBot(Bot):
            ...
            for template in ignoreTemplates[page.site.code]:
                if not isinstance(template, tuple):
                    for pageTemplate in templatesInThePage:
                        if pageTemplate.title(with_namespace=False) == template:
                            return True
                else:
                    for (inPageTemplate, param) in templatesWithParams:
                        if inPageTemplate.title(with_namespace=False) == template[0] \
    ...
    def changeCommonscat(self, page=None, oldtemplate=u'', oldcat=u'',
        ...
        if not linktitle and (page.title().lower() in oldcat.lower() or
            ...
            linktitle = oldcat
        if linktitle and newcat != page.title(with_namespace=False):
            newtext = re.sub(r'(?i)\{\{%s\|?[^{}]*(?:\{\{.*\}\})?\}\}'
                             % oldtemplate,
                             u'{{%s|%s|%s}}' % (newtemplate, newcat, linktitle),
                             page.get())
        elif newcat == page.title(with_namespace=False):
            newtext = re.sub(r'(?i)\{\{%s\|?[^{}]*(?:\{\{.*\}\})?\}\}'
                             % oldtemplate,
                             u'{{%s}}' % newtemplate,
                             page.get())
    ...
    def getCommonscatLink(self, wikipediaPage=None):
        ...
        for template in wikipediaPage.templatesWithParams():
            templateTitle = template[0].title(with_namespace=False)
            if templateTitle == primaryCommonscat \
               or templateTitle in commonscatAlternatives:
                ...
                if (len(template[1]) > 0):
                    ...
                    commonscatTarget = wikipediaPage.title(
        ...
        try:
            ...
            if not commonsPage.exists():
                ...
                return self.checkCommonscatLink(
                    commonsPage.getRedirectTarget().title(with_namespace=False))
                ...
                for template in commonsPage.templatesWithParams():
                    if (template[0].title(
                        with_namespace=False) == "Category redirect" and
                            len(template[1]) > 0):
                ...
                return commonsPage.title(with_namespace=False)
        except pywikibot.BadTitle:
