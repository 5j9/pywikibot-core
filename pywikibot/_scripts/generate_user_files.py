...
from os.path import join, dirname, abspath, exists, isfile, split
from textwrap import fill


_scripts_dir = dirname(abspath(__file__))
...
def parse_sections():
    ...
    with codecs.open(
        join(pywikibot_dir, 'config2.py'), 'r', 'utf-8'
    ) as config_f:
...
def create_user_config(main_family, main_code, main_username, force=False):
    ...
    """
    _fnc = join(base_dir, USER_BASENAME)
    _fncpass = join(base_dir, PASS_BASENAME)
...
def ask_for_dir_change(force):
    ...
    while True:
        ...
        userfile = file_exists(join(base_dir, USER_BASENAME))
        passfile = file_exists(join(base_dir, PASS_BASENAME))
