...
class ProofreadPage(pywikibot.Page):
    ...
    def _parse_title(self):
        ...
        @rtype: tuple
        """
        left, sep, right = self.title(with_namespace=False).rpartition('/')
        if sep:
            base = left
            num = int(right)
        else:
            ...
            if not what_links_here:
                ...
                if self._num is not None:
                    for page in what_links_here:
                        if page.title(with_namespace=False) == self._base:
    ...
    def _do_hocr(self):
        ...
        params = {'book': self.title(as_url=True, with_namespace=False),
                  'lang': self.site.lang,
                  'user': self.site.user(),
                  }
